...
def get_args():
    ...
    parser.add_argument('--model_path', default='output/train_open_InternVId_256_500000images_resume_1/checkpoints/epoch_13_step_251000.pth', type=str)
    ...
    parser.add_argument('--local_rank', type=int, default=0, help='Local rank passed from deepspeed launcher')
...
def visualize(items, bs, sample_steps, cfg_scale):
    ...
    for chunk in tqdm(list(get_chunks(items, bs)), unit='batch'):
        ...
        for i, sample in enumerate(samples):
            ...
            img = save_image(sample, save_path, nrow=1, normalize=True, value_range=(-1, 1))
            video.append(img)
...
if __name__ == '__main__':
    ...
    print("args.t5_path",args.t5_path)
    import time
    s = time.time()
