...
from diffusion.model.nets.blocks import (
    Attention,
    CaptionEmbedder,
...
@MODELS.register_module("PixArtT2V_XL_2",force=True)
def PixArtT2V_XL_2(**kwargs):
    model = PixArtT2V3(depth=28, hidden_size=1152, patch_size=(1, 2, 2), num_heads=16, **kwargs)
...
def PixArtMS_XL_2(**kwargs):
    model = PixArtMSV2(depth=28, hidden_size=1152, patch_size=(1, 2, 2), num_heads=16, **kwargs)
    # if from_pretrained is not None:
    #     load_checkpoint(model, from_pretrained)
    return model
...
if __name__ == "__main__":
    ...
    print(model)
    x = torch.randn(1,4,16,32,32).cuda()
    t = torch.randn(1).cuda()
    y = torch.randn(1,1,120,4096).cuda()
    y = model(x,t,y)
